...
@section('content')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" class="step-1" action="{{route('SitesModule::sites.create.step-2')}}">
                            @csrf
                            ...
                            <div class="form-group @error('status') has-danger @enderror">
                                ...
                                <select name="status" id="" class="select form-control">
                                    @foreach($statuses as $status)
                                        <option value="{{$status}}" @if(old('status') === $status) selected @endif>{{__('SitesModule::statuses.'.$status)}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group @error('site_type_id') has-danger @enderror">
                                ...
                                <select name="site_type_id" id="" class="select form-control">
                                    ...
                                    @foreach($siteTypes as $siteType)
                                        <option value="{{$siteType->id}}" @if((int) old('site_type_id') === $siteType->id) selected @endif>{{$siteType->name}}</option>
                                    @endforeach
                                </select>
    ...
    @javascript('structure', json_decode(old('structure')))
@endsection
